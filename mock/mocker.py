...
COLLECTION_NAMES = ["user", "svcp", "booking", "chat"]  # collection name
    ...
    for key in properties_dict:
        ...
        if t == "string":
            if "pattern" in properties_dict[key]:
                limit = (
                    20
                    if "maxLength" not in properties_dict[key]
                    else properties_dict[key]["maxLength"]
                )
                ret[key] = exrex.getone(properties_dict[key]["pattern"], int(limit))
            elif "password" in key.lower():
                ret[key] = (
                    r"$2a$10$brrTzHVOuWleFQFlYqeU.eZbQXySNemw6d8.dYPCBxRMBRM5NCMy."  # 'password' that encrypted
                )
            else:
                ret[key] = key + str(id)
            if (
                "minLength" in properties_dict[key]
                and len(ret[key]) < properties_dict[key]["minLength"]
            ):
                ret[key] = ret[key] + "0" * (
                    properties_dict[key]["minLength"] - len(ret[key])
                )
        elif t == "date":
            ...
            minimum = (
                0
                if "minimum" not in properties_dict[key]
                else properties_dict[key]["minimum"]
            )
            maximum = (
                100
                if "maximum" not in properties_dict[key]
                else properties_dict[key]["maximum"]
            )
            ...
            print(
                "error (type=" + properties_dict[key]["bsonType"] + ") not implemented"
            )
